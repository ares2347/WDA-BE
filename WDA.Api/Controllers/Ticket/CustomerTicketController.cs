...
public class CustomerTicketController : ControllerBase
{
    ...
    public CustomerTicketController(IUnitOfWork unitOfWork, IMapper mapper, IEmailService emailService,
        UserContext userContext, UserManager<Domain.Models.User.User> userManager, IAuthorizationService authorizationService)
    {
        ...
        CancellationToken _)
    {
        try
        {
            var ticket = await _unitOfWork.TicketRepository
                .GetCustomerTickets(x => !x.IsDeleted && x.TicketId == request.TicketId && x.Status == TicketStatus.Opened, null, null)
        ...
        CancellationToken _)
    {
        try
        {
            var ticket = await _unitOfWork.TicketRepository.GetCustomerTickets(x =>
                    x.TicketId == request.TicketId && (x.Status == TicketStatus.Closed || x.Status == TicketStatus.Done), null, null)
                .FirstOrDefaultAsync(_);
            if (ticket is null) return NotFound("Ticket not found.");
            ...
            //send email customer
            var customerEmailTemplate = await _emailService.GetEmailTemplate(EmailTemplateType.TicketReopenedRequestor, _);
            var subject = customerEmailTemplate!.Subject.Replace("[[TicketId]]",
                res?.TicketId.ToString() ?? string.Empty);
            var customerReplacements = new Dictionary<string, string>
            {
            ...
            //
            //send email employee
            var employeeEmailTemplate = await _emailService.GetEmailTemplate(EmailTemplateType.TicketReopenedResolver, _);
            var employeeEmailSubject = employeeEmailTemplate!.Subject.Replace("[[TicketId]]",
                res?.TicketId.ToString() ?? string.Empty);
            var employeeReplacements = new Dictionary<string, string>
            {
